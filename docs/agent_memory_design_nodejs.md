...
- `LLMUserMessageReadyEventHandler` calls:
  - `context.state.llm_instance.streamMessages(...)` with assembled messages
...
- `streamMessages(messages: List[Message], **kwargs)`
...
- Add to `BaseLLM`:
  - `streamMessages(messages: List[Message], **kwargs)`
  - `sendMessages(messages: List[Message], **kwargs)`
...
ingestToolIntent(toolInvocation, turnId?): void
ingestToolResult(toolResultEvent, turnId?): void
ingestAssistantResponse(completeResponse, turnId, sourceEvent): void
requestCompaction(): void
clearCompactionRequest(): void
getRawTail(tailTurns, excludeTurnId?): RawTraceItem[]
getTranscriptMessages(): Message[]
resetTranscript(snapshotMessages): void
getToolInteractions(turnId?): ToolInteraction[]
```

### LLMRequestAssembler

```
prepareRequest(processedUserInput, currentTurnId?, systemPrompt?): RequestPackage
renderPayload(messages): ProviderPayload
```
