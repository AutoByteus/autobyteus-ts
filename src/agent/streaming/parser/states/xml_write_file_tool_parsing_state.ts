...
export class XmlWriteFileToolParsingState extends XmlToolParsingState {
  ...
  constructor(context: ParserContext, openingTag: string) {
    ...
    if (this.toolName !== undefined && this.toolName !== 'write_file') {
      // No-op: specialized state expects write_file but falls back gracefully.
    }
  }
  ...
  run(): void {
    ...
    if (!this.foundContentStart) {
      ...
      const contentMatch = /<arg\s+name=["']content["']>/i.exec(this.contentBuffering);
      if (contentMatch) {
  ...
  private processSeekMarkerContent(chunk: string): void {
    ...
    const startMarker = (this.constructor as typeof XmlWriteFileToolParsingState).START_CONTENT_MARKER;
    ...
    const closingIdx = this.contentSeekBuffer.indexOf((this.constructor as typeof XmlWriteFileToolParsingState).CONTENT_ARG_CLOSE_TAG);
    if (closingIdx !== -1) {
  ...
  private processDefaultContent(chunk: string): void {
    const closingTag = (this.constructor as typeof XmlWriteFileToolParsingState).CONTENT_ARG_CLOSE_TAG;
  ...
  private processMarkerContent(chunk: string): void {
    ...
    const endMarker = (this.constructor as typeof XmlWriteFileToolParsingState).END_CONTENT_MARKER;
    const closingTag = (this.constructor as typeof XmlWriteFileToolParsingState).CONTENT_ARG_CLOSE_TAG;
